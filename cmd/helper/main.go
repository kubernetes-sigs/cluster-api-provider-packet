...
func main() {
	...
	rootCmd := &cobra.Command{ //nolint:exhaustivestruct
		...
		PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
			if err := klogFlags.Set("log_file", logFile); err != nil {
				return fmt.Errorf("failed to configure klog log file: %w", err)
			}
			...
			if config.NoTUI {
				if err := klogFlags.Set("alsologtostderr", "true"); err != nil {
					return fmt.Errorf("failed to configure klog stderr output: %w", err)
				}
			}

			if err := klogFlags.Parse(nil); err != nil {
