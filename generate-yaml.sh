...
PROVIDER_TEMPLATE_FILE=${BASEDIR}/provider-components.yaml.template
PROVIDER_GENERATED_FILE=${OUTPUT_DIR}/provider-components.yaml
...
if [ -z "$PACKET_API_KEY" ]; then
  echo "Must specify the Packet API key as PACKET_API_KEY"
  exit 1
fi
